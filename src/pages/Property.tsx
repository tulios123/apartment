...
function PropertyForm({
  ...
  onSave: (address: string, notes: string) => Promise<void>
  onCancel: () => void
}) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setErr(null)
    try {
      await onSave(address.trim(), notes.trim())
    } catch (e) {
  ...
  return (
    <form onSubmit={submit} className="form">
      <div className="form-row">
        <label>כתובת</label>
        <input type="text" value={address} onChange={e => setAddress(e.target.value)} required autoFocus />
      </div>
      <div className="form-row">
        <label>הערות</label>
...
export default function PropertyPage() {
  ...
  // ── Property handlers ──
  async function handlePropertySave(address: string, notes: string) {
    if (!user) return
    if (property) {
      await updateProperty(property.id, { address, notes: notes || null })
    } else {
      await createProperty({ owner_id: user.id, address, notes: notes || null })
    }
